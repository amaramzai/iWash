...
struct GameScreen6: View {
    ...
    var body: some View {
        ...
        GeometryReader {
            ...
            NavigationStack {
                ZStack {
                    ...
                    if dirtOpacity[2] < 0.1 && !isCleanDirt[2]{
                        ...
                        Image("dirtLongShirt3")
                            .resizable()
                            .scaledToFit()
                            .frame(width: geometry.size.width * 0.08)
                            .modifier(AmzaiSpecialGesture(opacity: $dirtOpacity[2]))
                            .position(x :  CalculateX(xPos: 0.362), y : CalculateY(yPos: 0.44))
                    }
                    //dirt4
                    if dirtOpacity[3] < 0.1 && !isCleanDirt[3]{
                        CheckMarkPop()
                            .onAppear{
                                withAnimation(.easeInOut(duration: 2)) {
                                    checkMarkDirt[3] = CGPoint(x: checkMarkDirt[3].x + 0, y: checkMarkDirt[3].y - 100)
                                    self.counter += 1
                                }
                                Timer.scheduledTimer(withTimeInterval: 2.0, repeats: false, block: { timer in
                                    withAnimation(.spring()) {
                        ...
                        Image("dirtLongShirt4")
                            .resizable()
                            .scaledToFit()
                            .frame(width: geometry.size.width * 0.08)
                            .modifier(AmzaiSpecialGesture(opacity: $dirtOpacity[3]))
                        .position(x :  CalculateX(xPos: 0.5), y : CalculateY(yPos: 0.6))}
                    //dirt5
                    VStack {
                        Spacer()
                        NavigationLink(destination: GameScreen8()){
            ...
            checkMarkDirt[0] = CGPoint(x: CalculateX(xPos: 0.44), y: CalculateY(yPos: 0.365))
            checkMarkDirt[1] = CGPoint(x: CalculateX(xPos: 0.6), y: CalculateY(yPos: 0.43))
            checkMarkDirt[2] = CGPoint(x: CalculateX(xPos: 0.362), y: CalculateY(yPos: 0.44))
            checkMarkDirt[3] = CGPoint(x: CalculateX(xPos: 0.5), y: CalculateY(yPos: 0.6))
        }
    }
    ...
    //----------------------------------------------------
    struct GameScreen6_Previews: PreviewProvider {
